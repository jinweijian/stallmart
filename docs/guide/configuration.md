...
| 文件 | 说明 |
| --- | --- |
| `project.config.json` | 仓库根目录打开微信开发者工具时使用，`miniprogramRoot` 指向 `app/dist/`。 |
...
仓库内 `project.config.json` 和 `app/project.config.json` 只保留微信开发者工具占位 `appid: touristappid`。真实小程序 AppID 属于环境配置：本地可放在已忽略的 `project.private.config.json` 或通过 `WECHAT_APP_ID` 注入 Taro 构建；后端根据运行时传入的 AppID 映射店铺，不把真实 AppID 写入仓库。

H5 和小程序都走真实 API 数据链路。小程序 request 合法域名申请完成前，可先通过 H5 验证主题和数据链路：

微信开发者工具打开仓库根目录时，根目录 `project.config.json` 必须将 `miniprogramRoot` 指向 `app/dist/`；打开 `app/` 时，`app/project.config.json` 必须将 `miniprogramRoot` 指向 `dist/`，确保工具读取 Taro weapp 构建产物。若直接打开 `app/dist/`，产物内的 `project.config.json` 会使用 `miniprogramRoot: "./"`。

| 变量 | 用途 |
| --- | --- |
| `WECHAT_APP_ID` | 微信开发者工具/Taro 构建使用的真实小程序 AppID，不写入仓库。 |
