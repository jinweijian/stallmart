...
- 不在代码中写本机绝对路径。
- API endpoint 常量统一放在 `mini-program/src/app-config/index.ts`。
- 请求统一走 `mini-program/src/utils/request.ts`，不要在页面中直接调用 `Taro.request`。
...
- 新增接口时同步更新 Swagger 注解和 `docs/API.md`。
...
- 涉及端口、数据库名、外部服务地址的变更需要同步 `docs/CONFIGURATION.md`。
