...
| 页面路径 | 页面文件 | 模块 | 职责 |
| --- | --- | --- | --- |
| `pages/customer/index/index` | `src/pages/customer/index/` | 店铺浏览 | 扫码进入、店铺展示、商品浏览、商品详情弹层；用户视角可 SKU 加购，商家视角仅查看商品详情。 |
| `pages/customer/cart/cart` | `src/pages/customer/cart/` | 顾客端购物车 | 查看购物车、改数量、进入确认订单。 |
| `pages/customer/my-orders/my-orders` | `src/pages/customer/my-orders/` | 订单 / 接单 | 用户视角查看订单列表；商家视角作为接单页处理订单状态流转。 |
| `pages/customer/my/my` | `src/pages/customer/my/` | 我的 | 用户信息、登录状态、商家/用户视角切换和摊主入口。 |
...
| 购物车 | `pages/customer/cart/cart` | 用户视角为购物车；商家视角不进入购物车流程，误点时回到首页。 |
| 订单 | `pages/customer/my-orders/my-orders` | 用户视角为我的订单；商家视角为接单管理。 |
| 我的 | `pages/customer/my/my` | 用户信息、商家/用户视角切换与摊主入口。 |
...
- 当前身份由“我的”页在本地持久化为用户视角或商家视角，切换控件在未登录状态下也必须可见；切回用户视角后购物车、下单、我的订单按普通用户流程运行。
...
- `pages/vendor` 分包的摊位总览、订单管理和摊位设置必须读取同一份顾客端主题变量，保持与当前店铺风格一致。
...
- 商家视角下首页不展示底部购物车栏、商品加购按钮和加购提交栏，商品行点击仍打开详情弹层并展示规格、状态、库存和销量。
- 购物车负责商品集合管理，不负责用户认证细节。
- 商家视角不使用购物车；如果从原生 tabBar 进入购物车页，页面提示后返回首页。
...
- 我的订单负责订单查询和状态展示，使用 `pageThemes.orders` 的 banner 和状态色，不新增独立订单详情路由；订单页顶部活动 banner 使用 `promoBanner` 展示尺寸，避免 `692rpx x 220rpx` 素材被压缩；订单卡必须常显取餐码，底部必须预留原生 tabBar 和安全区，避免最后一张订单被遮挡。
- 同一个订单 tab 在商家视角下作为接单页，读取 `/vendor/orders`，支持 `NEW -> ACCEPTED -> PREPARING -> READY -> COMPLETED`，并允许新订单拒单。
- 我的页面使用 `pageThemes.my` 的欢迎文案、邀请 banner 和菜单 icon，保留现有登录、绑定手机、订单统计、商家/用户视角切换、摊主入口、常见问题、客服、关于我们；当前不展示会员、优惠券、卡包、礼品、成长值。
...
- 商家视角用于当前小程序内快速管理；前端可切换展示，但真实摊主权限仍必须由后端兜底。
- 摊主订单操作必须遵循订单状态机。
- 摊位设置保存后必须刷新我的摊位页面数据。
