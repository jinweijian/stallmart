...
| 店铺覆盖 | `StoreDecorationDTO` | 第一版商家只维护当前店铺的 Logo、封面、banner 和展示描述；历史覆盖字段保留给兼容和平台迁移，不在商家端开放。 |
...
平台角色负责创建、修改、上传素材、上下架和删除未使用的风格包；已被店铺引用的风格包不能硬删，只能下架来阻止后续选择。商家角色第一版只能选择已上架风格包，服务端必须拒绝商家切换到下架风格包，也必须拒绝商家通过装修接口提交颜色、文案、图标、分类 icon 库或主题图片覆盖字段。后台上传素材后保存网络 URL，小程序运行时按配置读取，不把商家素材重新打包到 `app/src/static`。
