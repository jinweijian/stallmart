...
- 线上平台账号密码调整为 `platform / stallmart&p@2026..`。
- 线上商家账号密码调整为 `vendor / stallmart&v@2026..`。
...
- 验证码由服务端生成简单算术题，返回 `captchaId` 和题目文本，答案只保存在服务端内存状态中并设置短 TTL。
...
- `GET /admin/auth/captcha` 返回 `{ captchaId, question }`。
